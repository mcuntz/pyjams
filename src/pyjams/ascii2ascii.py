...
def ascii2ascii(edate, full=False, en=False, fr=False, us=False,
    ...
    *YYYY-MM-DD hh:mm:ss*, American *MM/DD/YYYY hh:mm:ss*, and French *DD/MM/YYYY
    ...
    YY : bool, optional
        True: year in input file is 2-digit year. Every year that is above the
        current year will be taken as being in 1900 (default: False).

    Returns
    -------
    array_like
        date strings in chosen date format.
        If no optional keyword is True then output is in ascii format:
        *DD.MM.YYYY hh:mm:ss*.
        ...
        for i, d in enumerate(idate):
            if en:
                if '-' in d:
                    ...
                    if int(d[6:8]) > iyr2:                         # us->en
                        dd = '19'+d[6:8]+'-'+d[0:2]+'-'+d[3:5]+d[8:]
                    else:
                        dd = '20'+d[6:8]+'-'+d[0:2]+'-'+d[3:5]+d[8:]
                else:
                    if int(d[6:8]) > iyr2:                         # ascii->en
                        dd = '19'+d[6:8]+'-'+d[3:5]+'-'+d[0:2]+d[8:]
                    else:
    ...
    if full:
        odate = [ (d+' 00:00:00')[:19] if len(d) < 11 else (d+':00:00')[:19]
                  for d in odate ]
...
def ascii2en(edate, **kwarg):
    """
    Wrapper function for :func:`ascii2ascii` with English date format output

    That means :func:`ascii2ascii` with *en=True* -
...
def ascii2fr(edate, **kwarg):
    """
    Wrapper function for :func:`ascii2ascii` with French date format output

    That means :func:`ascii2ascii` with *fr=True* -
...
def ascii2us(edate, **kwarg):
    """
    Wrapper function for :func:`ascii2ascii` with US-English date format output

    That means :func:`ascii2ascii` with *us=True* -
...
def en2ascii(edate, **kwarg):
    ...
    That means `ascii2ascii(edate, **kwarg)`.
...
def fr2ascii(edate, full=False, YY=False):
    """
    Convert French date notation *DD/MM/YYYY hh:mm:ss* to ascii notation

    That means *DD/MM/YYYY hh:mm:ss* to *DD.MM.YYYY hh:mm:ss*.
    Simply replaces '/' with '.', assuring the same iterable type as the input.
    ...
    if YY:
        ...
        iyr2 = int(ptime.asctime()[-2:])
        odate = [ d[0:6]+'19'+d[6:]
                  if int(d[6:8]) > iyr2 else d[0:6]+'20'+d[6:]
                  for d in odate ]  # ascii->ascii
...
def us2ascii(edate, **kwarg):
    ...
    That means :func:`ascii2ascii` with *us=True* -
    `ascii2ascii(edate, ur=True, **kwarg)`.
